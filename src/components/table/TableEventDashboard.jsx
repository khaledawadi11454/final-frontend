...
import Edit from "./edit/editEvent";
import Add from "./add/AddEvents";

const TableEventDashboard = () => {
  ...
  const getData = async () => {
    try {
      setLoading(true);
      let res = await axios.get(`${process.env.REACT_APP_URL}/event`);
  ...
  const handleDelete = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios
          .delete(`${process.env.REACT_APP_URL}/event/delete/${id}`)
          .then((response) => {
            console.log(response);
            getData();
          })
